...
while b_running:
    ...
    while b_running:
        try:
            ...
            #print('t = ', t)
            #(trans,rot) = listener.lookupTransform(source_link, dest_link, t)
            (trans,rot) = listener.lookupTransform(source_link, dest_link, rospy.Time(0))
        except tf.Exception as e :
            print('Exception: ', e)
        # rate.sleep()
        if b_read:
            b_read, b_move_end = False, False
            r = 10
            i = [round(uvd[0], r), round(uvd[1], r), round(uvd[2], r)] #image
            w = [round(trans[0], r), round(trans[1], r), round(trans[2], r)] #world
            print(i, '->', w)
            text = str(i[0]) + ', ' + str(i[1]) + ', ' + str(i[2]) + ', ' + str(w[0]) + ', ' + str(w[1]) + ', ' + str(w[2]) + '\n'
            print(text)
            f.write(text)
            break
            #print(text)
