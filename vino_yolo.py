...
pub_uvd = rospy.Publisher('uvd', Float32MultiArray, queue_size=1)  #Publish(u,v and depth)
rospy.init_node('camera_detector', anonymous=True)

# h = imgsz[0]
# w = imgsz[1]
fourcc = cv2.VideoWriter_fourcc(*'XVID')
out = cv2.VideoWriter('/home/installer/ros_ws/videos/iros_d77_calib2.avi', fourcc, 15, (1024, 281))

#Open save file or received from other PCs
intrinsic = np.load('/home/installer/ros_ws/src/detector/scripts/param/intrinic.npy')
distortion = np.load('/home/installer/ros_ws/src/detector/scripts/param/dis.npy')
R_mtx = np.load('/home/installer/ros_ws/src/detector/scripts/param/R_mtx.npy')
tran = np.load('/home/installer/ros_ws/src/detector/scripts/param/tran.npy')
...
def get_trans():
    ...
    while True:
        ...
        try:
            #base_link
            (trans,rot) = listener.lookupTransform('/base_link_d435', '/left_hand', rospy.Time(0))
...
class YOLO_OPENVINO(object):
    ...
    def draw(self, img_tuple, boxinfo):
        ...
        for xyxy, conf, cls in boxinfo:
            ...
            distance = dist + m_delta - 0.03
            base = 0.67
            scale = (distance)*coeff[0] + coeff[1]
            # print(f'distance = {distance}, scale = {scale}')
            ...
            XYZ[2][0] = distance
            z, y, x = XYZ[0][0], XYZ[1][0], XYZ[2][0]
            
            if cls == 2:
                # self.plot_one_box(xyxy, depth_frame, label=f'({x:.2f},{y:.2f},{z:.2f})',  color=self.colors[int(cls)], line_thickness=2)
                self.plot_one_box(xyxy, depth_frame, label=f'depth = {distance:.2f}m',  color=self.colors[int(cls)], line_thickness=2)
                c1, c2 = (x_center-box_size, y_center-box_size), (x_center+box_size, y_center+box_size)
                cv2.rectangle(depth_frame, c1, c2, color=(255,0,0), thickness=1)
            
            uvd = [x_center, y_center, dist]
...
if __name__ == "__main__":
    ...
    #args.model = f"/home/installer/yolo/shapes_3d_{args.im_size}.onnx"

    args.model = "/home/installer/Downloads/yolov8_1/models/v8_1000_640.onnx"

    #args.model = f"/home/installer/yolo/old_models/yolo_tiny_3dshapes.onnx"
    yolo_detector= YOLO_OPENVINO(args.model, args.device, args.pre_api, args.batchsize, args.nireq, args.im_size)
    yolo_detector.infer_cam()
